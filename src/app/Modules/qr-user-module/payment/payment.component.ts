...
export class PaymentComponent implements OnInit {
  ...
  async initializeProperty(): Promise<void> {
    ...
    this.identity = navigation?.extras?.state?.['id'] || await this.sharedService.getItem('clientId');
  ...
  createOrders(): void {
    ...
    this.loginService.createOrder(this.username, amount).subscribe(
      (response) => {
  ...
  verifyPayment(paymentId: string, orderId: string, signature: string): void {
    ...
    this.loginService.verifyPayment(paymentId, orderId, signature).subscribe(
      (response) => {
